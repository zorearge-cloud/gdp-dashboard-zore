...
def apply_cosmic_style(fig, chart_type="bar"):
    ...
    # Güvenli zaman ekseni sabitlemesi
...
if page == "1. Genel Dashboard":
    ...
    if df_dashboard.empty:
        ...
        g1.plotly_chart(apply_cosmic_style(fig1, "vertical_bar"), use_container_width=True)
        ...
        g2.plotly_chart(apply_cosmic_style(fig2, "vertical_bar"), use_container_width=True)
        ...
        if not aylar:
            ...
            # Doğuştan kararlı yapay zeka animasyonu motoru (Çökmeyi önlemek için güvenli şema)
            fig3_live = px.bar(
                df_race_dense,
                x='TOPLAM_SERMAYE',
                y='FIRMA',
        ...
        g8.plotly_chart(apply_cosmic_style(fig8, "vertical_bar"), use_container_width=True)
    ...
    if not firmalar: st.warning("Aktör bulunamadı.")
        ...
        if not firma_df.empty and firma_df['TOPLAM_SERMAYE'].sum() > 0:
            ...
            fig_a.update_layout(title=f"{selected_firma} Ürün Kategorisi Dağılımı (İlk 6 + Diğer)")
        ...
        if not trend_data_all.empty and trend_data_all['TOPLAM_SERMAYE'].sum() > 0:
            # Buradaki eksen hatası giderildi (xaxis_type yerine update_xaxes kategori yapıldı)
            fig_b = px.bar(trend_data_all, x='SIPARIS_AY', y='TOPLAM_SERMAYE', title=f"{selected_firma} Dönemsel Alım Trendi ($)", color_discrete_sequence=['#00f3ff'])
            col_b.plotly_chart(apply_cosmic_style(fig_b, "vertical_bar"), use_container_width=True)
            
        st.markdown("---")
        st.subheader(f"🔍 {selected_firma} Sipariş Listesinde Barkod Sorgulama")
        search_barcode = st.text_input("Barkod Yazın (Var mı / Yok mu Kontrolü):", placeholder="Kontrol etmek istediğiniz barkodu buraya girin...").strip()
        display_df = firma_df.copy()
        if search_barcode:
            search_res = display_df[display_df['BARKOD'].str.contains(search_barcode, case=False, na=False)]
            if not search_res.empty:
                st.success(f"🚀 VERİ DOĞRULANDI: {len(search_res)} kayıt izole edildi!")
                display_df = search_res  
            else: st.error("🚨 SİNYAL YOK: Barkod bu aktörün sicilinde mevcut değil.")
        ...
        st.write(f"{selected_firma} Veri Listesi:")
