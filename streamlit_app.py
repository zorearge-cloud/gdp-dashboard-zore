...
import datetime
import json
import re  # Veri temizleme motoru için kritik kütüphane sabitlendi.

# --- KURUMSAL PANEL AYARLARI ---
st.set_page_config(layout="wide", page_title="Zore Yönetim Paneli")

# Arka Plan ve Arayüz Stabilizasyonu (CSS)
st.markdown("""
<style>
    ...
    for idx, link in enumerate(LINKS):
        try:
            ...
            if res.status_code != 200: 
                error_logs.append(f"Link {idx+1} HTTP Bağlantı Hatası: {res.status_code}")
                continue
            ...
            error_logs.append(f"Link {idx+1} Dosya Okuma Hatası: {str(e)}")
            continue
    return (pd.concat(master_list, ignore_index=True) if master_list else pd.DataFrame()), error_logs

# Ana Veri Yükleme Adımı
df_raw, logs = load_dashboard_data()

if logs:
    with st.sidebar.expander("📋 Sistem Sistem Logları"):
        for log in logs: st.caption(log)

if df_raw.empty:
    st.error("⚠️ Veri matrisi yüklenemedi. Lütfen bağlantılarınızı ve Excel sayfa isimlerini kontrol edin.")
else:
    # 2026 Filtreleme Kuralları Korundu
    ...
    # Orijinal Kurumsal Metrik Hesaplamaları
    ...
    # --- ESKİ KURUMSAL İSİMLER & YENİ GÖRSEL TASARIM (ECHARTS CHASSIS) ---
    dashboard_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <div class="header-matrix">
            <div class="matrix-card" style="border-color: rgba(0, 243, 255, 0.45);">
                ...
                <p class="card-value" style="color: #00f3ff;">{total_adet:,} <span style="font-size:16px; color:#5f7595;">Adet</span></p>
            </div>
            <div class="matrix-card" style="border-color: rgba(255, 0, 255, 0.45);">
                <p class="card-title">💰 TOPLAM SERMAYE / CİRO</p>
            ...
            <div class="matrix-card" style="border-color: rgba(0, 255, 102, 0.45);">
                <p class="card-title">🏢 AKTİF FİRMA SAYISI</p>
        ...
        <script>
            ...
            // --- 1. FİRMA BAZLI SERMAYE DAĞILIMI (BAR) ---
            const barChart = echarts.init(document.getElementById('glow_bar'));
            barChart.setOption({{
                backgroundColor: 'transparent',
                title: {{ text: '// Firma Bazlı Sermaye Dağılımı (İlk 7)', textStyle: {{ color: '#00f3ff', fontSize: 13, fontWeight: 'normal' }} }},
                tooltip: {{ trigger: 'axis', backgroundColor: 'rgba(6,16,39,0.9)', borderColor: '#00f3ff' }},
            ...
            // --- 2. TÜR BAZLI HARCAMA DAĞILIMI (DONUT) ---
            const pieChart = echarts.init(document.getElementById('glow_pie'));
            pieChart.setOption({{
                backgroundColor: 'transparent',
                title: {{ text: '// Ürün Kategorisi / Tür Dağılımı', textStyle: {{ color: '#ff00ff', fontSize: 13, fontWeight: 'normal' }} }},
            ...
            // --- 3. DÖNEMSEL ALIM/SERMAYE TRENDİ (LINE) ---
            const lineChart = echarts.init(document.getElementById('glow_line'));
            lineChart.setOption({{
                backgroundColor: 'transparent',
                title: {{ text: '// Dönemsel Sermaye Akış Trendi', textStyle: {{ color: '#00ff66', fontSize: 13, fontWeight: 'normal' }} }},
    ...
    # HTML Çıktıyı Streamlit Üzerinde Render Ediyoruz
    st.components.v1.html(dashboard_html, height=730, scrolling=False)